<?php
    ...
    $cons_alert_prod= "SELECT 
                            CASE 
                            WHEN TO_CHAR(SYSDATE,'DD/MM/YYYY') 
                            BETWEEN (SELECT TO_CHAR(MAX(sol.HR_CADASTRO),'DD/MM/YYYY') AS ULT_SOL
                                    FROM portal_sesmt.SOLICITACAO sol
                                    WHERE sol.CD_PRODUTO_MV = $var_prodto_alert
                                    AND sol.CD_USUARIO_MV = '$var_usu_alert') 
                            AND TO_CHAR((SELECT MAX(sol.HR_CADASTRO)AS ULT_SOL
                                        FROM portal_sesmt.SOLICITACAO sol
                                        WHERE sol.CD_PRODUTO_MV = $var_prodto_alert
                                        AND sol.CD_USUARIO_MV = '$var_usu_alert')
                                        + 
                                        (SELECT lgdur.DIAS
                                            ...
                                            WHERE lgdur.CD_DURABILIDADE IN (SELECT so.CD_DURABILIDADE
                                                                            FROM portal_sesmt.SOLICITACAO so
                                                                            WHERE so.CD_SOLICITACAO IN (SELECT MAX(sol.CD_SOLICITACAO)AS ULT_SOL
                                                                                                    FROM portal_sesmt.SOLICITACAO sol
                                                                                                    WHERE sol.CD_PRODUTO_MV = $var_prodto_alert
                                                                                                    AND sol.CD_USUARIO_MV = '$var_usu_alert'))
                                            UNION ALL
                                            ...
                                            WHERE dur.CD_DURABILIDADE IN (SELECT so.CD_DURABILIDADE
                                                                        FROM portal_sesmt.SOLICITACAO so
                                                                        WHERE so.CD_SOLICITACAO IN (SELECT MAX(sol.CD_SOLICITACAO)AS ULT_SOL
                                                                                                    FROM portal_sesmt.SOLICITACAO sol
                                                                                                    WHERE sol.CD_PRODUTO_MV = $var_prodto_alert
                                                                                                    AND sol.CD_USUARIO_MV = '$var_usu_alert'))),'DD/MM/YYYY')
                            THEN 1
                            ELSE 0
                            END SN_ALERTA_DUR
                            FROM DUAL";
